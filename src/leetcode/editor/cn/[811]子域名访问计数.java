package leetcode.editor.cn;//网站域名 "discuss.leetcode.com" 由多个子域名组成。顶级域名为 "com" ，二级域名为 "leetcode.com" ，最低一级为
...
// 
//
// Related Topics 数组 哈希表 字符串 计数 👍 114 👎 0
...
class subdomainVisits811 {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cpdomain : cpdomains) {
            ...
            StringBuilder builder = new StringBuilder(domains[domains.length - 1]);
            cnts.put(builder.toString(), cnts.getOrDefault(builder.toString(), 0) + cnt);
            for (int i = domains.length - 2; i >= 0; i--) {
                builder.insert(0, ".");
                builder.insert(0, domains[i]);
                cnts.put(builder.toString(), cnts.getOrDefault(builder.toString(), 0) + cnt);
            }
        }
