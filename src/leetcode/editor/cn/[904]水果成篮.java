package leetcode.editor.cn;//你正在探访一家农场，农场从左到右种植了一排果树。这些树用一个整数数组 fruits 表示，其中 fruits[i] 是第 i 棵树上的水果 种类 。
//
...
class FruitsBucket904 {
    public int totalFruit(int[] fruits) {
        ...
        while (r <= fruits.length) {
            ...
            while (r < fruits.length && cnt.size() <= 2) {
                if (cnt.size() == 2 && !cnt.containsKey(fruits[r])) {
                    break;
                } else {
